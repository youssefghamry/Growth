...
if ( ! class_exists( 'Wp_rem_arrange_viewing_status_updated_email_template' ) ) {
	...
	class Wp_rem_arrange_viewing_status_updated_email_template {
		...
		function get_arrange_viewing_user_name() {
			$arrange_viewing_member = get_post_meta( $this->viewing_id, 'wp_rem_viewing_member', true );
			return esc_html(get_the_title( $arrange_viewing_member ));
		}
		function get_arrange_viewing_user_email() {
			$arrange_viewing_member = get_post_meta( $this->viewing_id, 'wp_rem_viewing_member', true );
			$viewing_member_email_address = get_post_meta($arrange_viewing_member, 'wp_rem_email_address', true);
			return $viewing_member_email_address;
		}
		function get_property_user_name() {
			$property_member = get_post_meta( $this->viewing_id, 'wp_rem_property_member', true );
			return esc_html(get_the_title( $property_member ));
		}
		function get_property_user_email() {
			$property_member = get_post_meta( $this->viewing_id, 'wp_rem_property_member', true );
			$property_member_email_address = get_post_meta($property_member, 'wp_rem_email_address', true);
			return $property_member_email_address;
		}
		...
		function get_arrange_viewing_link() {
			...
			if( $member_dashboard != '' ){
				return wp_rem_wpml_lang_page_permalink( $member_dashboard, 'page' ). '?dashboard=viewings';
			}else{
