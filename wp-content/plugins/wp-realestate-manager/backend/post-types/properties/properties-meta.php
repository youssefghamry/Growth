...
if (!class_exists('wp_rem_property_meta')) {
	...
	public function __construct() {
	    ...
	    add_action('save_post', array($this, 'wp_rem_property_locations'), 11);
	    ...
	    add_action('save_post', array($this, 'wp_rem_save_search_keywords_field'), 20, 3);
	    ...
	    add_action('transition_post_status', array($this, 'wp_rem_move_property_trash'), 20, 3);
            ...
            //add_action( 'before_delete_post', array($this, 'before_delete_post_callback'));
	}
        
        public function before_delete_post_callback($post){
            pre($post);
        }
	    ...
	    <div class="page-wrap page-opts left" style="overflow:hidden; position:relative;">
	    	...
	    	<div class="opt-conts">
	    		...
	    		<nav class="admin-navigtion">
	    			...
	    			<li><a href="javascript:void(0);" name="#tab-detail-page-settings" href="javascript:;"><i class="icon-pencil2"></i> <?php echo wp_rem_plugin_text_srt('wp_rem_property_type_meta_det_page_elements'); ?></a></li>
				    <?php do_action('property_options_sidebar_tab'); ?>
	    		    </ul>
	    		</nav>
	    		<div id="tabbed-content" data-ajax-url="<?php echo esc_url(admin_url('admin-ajax.php')); ?>">
				    ...
				    <?php $this->wp_rem_single_page_options(); ?>
	    		    </div>
				<?php do_action('property_options_tab_container'); ?>
	    		</div>
	...
	function wp_rem_property_options() {
	    ...
	    $property_type_post = get_posts(array('posts_per_page' => '1', 'post_type' => 'property-type', 'name' => "$property_type_slug", 'post_status' => 'publish', 'suppress_filters' => false));
	    ...
	    echo '<div id="wp-rem-property-type-video-con" style="display: ' . ($type_video == 'on' ? 'block' : 'none') . ';">';
	    ...
	    echo '<div id="wp-rem-property-type-virtual-tour-con" style="display: ' . ($type_virtual_tour == 'on' ? 'block' : 'none') . ';">';
	    $wp_rem_html_fields->wp_rem_heading_render(
		    array(
			'name' => wp_rem_plugin_text_srt('wp_rem_add_property_virtual_tour'),
		...
		$wp_rem_html_fields->wp_rem_heading_render(
			array(
			    'name' => wp_rem_plugin_text_srt('wp_rem_property_locations_settings'),
	...
	function wp_rem_single_page_options() {
	    global $post, $wp_rem_form_fields, $wp_rem_form_fields, $wp_rem_html_fields, $wp_rem_plugin_options;
	    $post_id = $post->ID;
	    ...
	    $wp_rem_opt_array = array(
		'name' => wp_rem_plugin_text_srt('wp_rem_add_property_visibility'),
		'desc' => '',
		'hint_text' => '',
		'echo' => true,
		'field_params' => array(
		    'std' => '',
		    'id' => 'property_visibility',
		    'classes' => 'chosen-select-no-single',
		    'options' => array('public' => wp_rem_plugin_text_srt('wp_rem_add_property_public'), 'invisible' => wp_rem_plugin_text_srt('wp_rem_add_property_invisible')),
		    'return' => true,
		),
	    );
	    $wp_rem_html_fields->wp_rem_select_field($wp_rem_opt_array);
	    ...
	    $wp_rem_opt_array = array(
		'name' => wp_rem_plugin_text_srt('wp_rem_plugin_property_sold'),
		'desc' => '',
		'hint_text' => '',
		'echo' => true,
		'field_params' => array(
		    ...
		    'classes' => 'chosen-select-no-single',
		    'return' => true,
		    'options' => array('no' => wp_rem_plugin_text_srt('wp_rem_no'), 'yes' => wp_rem_plugin_text_srt('wp_rem_yes')),
		),
	    );
	    $wp_rem_html_fields->wp_rem_select_field($wp_rem_opt_array);

	    echo '<div id="wp-rem-property-features-btn-holder" style="display: ' . ($type_features == 'on' ? 'block' : 'none') . ';">';
	    $wp_rem_opt_array = array(
	    ...
	    echo '<div id="wp-rem-property-video-holder" style="display: ' . ($type_video == 'on' ? 'block' : 'none') . ';">';
	    $wp_rem_opt_array = array(
	    ...
	    echo '<div id="wp-rem-property-yelp-holder" style="display: ' . ($type_yelp_places == 'on' ? 'block' : 'none') . ';">';
	    $wp_rem_opt_array = array(
	    ...
	    echo '<div id="wp-rem-property-appartment-holder" style="display: ' . ($type_appartments == 'on' ? 'block' : 'none') . ';">';
	    $wp_rem_opt_array = array(
	    ...
	    echo '<div id="wp-rem-property-attachments-holder" style="display: ' . ($type_attachments == 'on' ? 'block' : 'none') . ';">';
	    $wp_rem_opt_array = array(
	    ...
	    echo '<div id="wp-rem-property-floor-plan-holder" style="display: ' . ($type_floor_plans == 'on' ? 'block' : 'none') . ';">';
	    $wp_rem_opt_array = array(
	...
	public function property_type_change_fields($property_type_slug = 0, $post_id = 0) {
	    ...
	    $html_val = '';
	    $html_val .= apply_filters('wp_rem_floor_plans_documnets_admin_fields', $post_id, $property_type_slug);
	    if (isset($html_val) && $html_val = $post_id) {
		$html .= '';
	    } else {
		$html .= $html_val;
	    }
		...
		echo json_encode(array('property_fields' => $html, 'detail_options' => $detail_page_options));
		die;
	    } else {
	...
	function wp_rem_save_property_price_type() {
	    ...
	    do_action('homevillas_save_variant_prices', $property_id, $price_type, $price);
	}
	...
	function property_price($property_type_slug = 0, $post_id = 0) {
	    ...
	    $property_type_post = get_posts(array('fields' => 'ids', 'posts_per_page' => '1', 'post_type' => 'property-type', 'name' => "$property_type_slug", 'post_status' => 'publish', 'suppress_filters' => false));
		...
		$html .= "
				...
				jQuery(document).ready(function() {
				    jQuery('#wp_rem_phone_number_property').parents('.form-elements').hide(); 
				       var val = jQuery('#wp_rem_property_price_options').val();
				       if(val == 'on-call')
				       {
				          jQuery('#wp_rem_phone_number_property').parents('.form-elements').show(); 
				       }
				});
		...
		$html = apply_filters('homevillas_extra_field_filter', $html, $post_id, $hide_div);
	...
	function property_categories($property_type_slug = 0, $post_id = 0, $backend = true) {
        ...
        $wp_rem_phone_number_property = get_post_meta($post_id, 'wp_rem_phone_number_property', true);
        $wp_rem_opt_array_ = array(
            'name' => wp_rem_plugin_text_srt('wp_rem_plugin_seetings_mobile_hint'),
            'desc' => '',
            'echo' => false,
            'field_params' => array(
                'std' => $wp_rem_phone_number_property,
                'cust_id' => 'wp_rem_phone_number_property',
                'cust_name' => 'wp_rem_phone_number_property',
                'return' => true,
            ),
        );

        //$html .= $wp_rem_html_fields->wp_rem_text_field($wp_rem_opt_array_);
	...
	function property_tags($property_type_slug = 0, $post_id = 0) {
	    ...
	    $wp_rem_property_type_tags = get_post_meta($post_id, 'wp_rem_property_tags', true);
	...
	function feature_fields($property_type_slug = 0, $post_id = 0) {
	    ...
	    $html = '<div id="wp-rem-property-features-holder" style="display: ' . ($type_features_element == 'on' ? 'block' : 'none') . ';">';
	    $html .= $wp_rem_html_fields->wp_rem_heading_render(
		    array(
	...
	public function wp_rem_property_locations($property_id = '') {
	    $wp_rem_property_location_country = wp_rem_get_input('wp_rem_post_loc_country_property', '', 'STRING');
	    $wp_rem_property_location_state = wp_rem_get_input('wp_rem_post_loc_state_property', '', 'STRING');
	    $wp_rem_property_location_city = wp_rem_get_input('wp_rem_post_loc_city_property', '', 'STRING');
	    $wp_rem_property_location_town = wp_rem_get_input('wp_rem_post_loc_town_property', '', 'STRING');
	    wp_set_post_terms($property_id, '', 'wp_rem_locations');
	    $location_ids = array();
	    if ($wp_rem_property_location_country) {
		$term = get_term_by('slug', $wp_rem_property_location_country, 'wp_rem_locations');
		$location_ids[] = $term->term_id;
	    }
	    if ($wp_rem_property_location_state) {
		$term = get_term_by('slug', $wp_rem_property_location_state, 'wp_rem_locations');
		$location_ids[] = $term->term_id;
	    }
	    if ($wp_rem_property_location_city) {
		$term = get_term_by('slug', $wp_rem_property_location_city, 'wp_rem_locations');
		$location_ids[] = $term->term_id;
	    }
	    if ($wp_rem_property_location_town) {
		$term = get_term_by('slug', $wp_rem_property_location_town, 'wp_rem_locations');
		$location_ids[] = $term->term_id;
	    }
	    wp_set_post_terms($property_id, $location_ids, 'wp_rem_locations');
	}

	/*
	 * Save data into search meta key for the property
	 * @ this will be called from the frontend on search
	 */

	public function wp_rem_save_search_keywords_field($property_id = '', $post = '', $update = '') {
	    $post_type = get_post_type($property_id);
	    if ($post_type == 'properties') {
		$post_title = $post->post_title;
		$content = $post->post_content;
		$property_summary = get_post_meta($property_id, 'wp_rem_property_summary', true);
		$categories = get_post_meta($property_id, 'wp_rem_property_category', true);
		$property_type = get_post_meta($property_id, 'wp_rem_property_type', true);
		$tags = get_post_meta($property_id, 'wp_rem_property_tags', true);

		$field_string = '';

		$field_string .= $post_title . ' | ';
		$field_string .= $content . ' | ';
		$field_string .= $property_summary . ' | ';

		if (!empty($categories)) {
		    foreach ($categories as $category_slug) {
			$term = get_term_by('slug', $category_slug, 'property-category');
			if (isset($term->name)) {
		...
		if (!empty($tags)) {
			...
			$term = get_term_by('slug', $tag_slug, 'property-tag');
			if (isset($term->name)) {
			    $field_string .= $term->name . ' | ';
			}
		    }
		...
		if ($property_type != '') {
		    $post_obj = get_page_by_path($property_type, OBJECT, 'property-type');
		    if (isset($post_obj->post_title)) {
			$field_string .= $post_obj->post_title . ' | ';
		    }
		}
		update_post_meta($property_id, 'search_keywords_field', $field_string);
	    }
	}
