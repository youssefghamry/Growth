...
if ( ! class_exists('wp_rem_opening_hours_element') ) {
	...
	class wp_rem_opening_hours_element {
		...
		public function __construct() {
			add_action('wp_rem_opening_hours_element_html', array( $this, 'wp_rem_opening_hours_element_html_callback' ), 11, 2);
		...
		public function wp_rem_opening_hours_element_html_callback($post_id, $op_view = '') {
						...
						<ul class="delivery-dropdown" style="display: none;">
							...
							foreach ( $opening_hours_list as $opening_hours_single_day_var => $opening_hours_single_day_val ) {
								...
								if ( $opening_hours_single_day_val['day_status'] == 'on' ) {
									?>
									<li><a href="javascript:void(0)"><span class="opend-day"><?php echo strtoupper(esc_html__($opening_hours_single_day_var, 'wp-rem')) ?></span> <span class="opend-time"><small>:</small> <?php wp_rem_plugin_text_srt('wp_rem_opening_hours_opens_at'); ?> <?php echo date_i18n('g:i a', strtotime($opening_hours_single_day_val['opening_time'])); ?> - <?php echo date_i18n('g:i a', strtotime($opening_hours_single_day_val['closing_time'])); ?></span></a></li>
									<?php
								} else {
									?>
									<li><a href="javascript:void(0)"><span class="opend-day"><?php echo strtoupper(esc_html__($opening_hours_single_day_var, 'wp-rem')) ?></span> <span class="close-day"><small>:</small> <?php echo wp_rem_plugin_text_srt('wp_rem_opening_hours_closed'); ?></span></a></li>
									<?php
								}
							}
							?>
						...
						<li>
							<ul class="delivery-dropdown">
								...
								foreach ( $opening_hours_list as $opening_hours_single_day_var => $opening_hours_single_day_val ) {
									...
									if ( $opening_hours_single_day_val['day_status'] == 'on' ) {
										?>
										<li class="<?php echo (strtoupper(date('D', current_time('timestamp', 1))) == strtoupper($opening_hours_single_day_var) ? 'today' : ''); ?>"><a href="#"><span class="opend-day"><?php echo strtoupper(esc_html__($opening_hours_single_day_var, 'wp-rem')) ?></span> <span class="opend-time"><small>:</small> <?php wp_rem_plugin_text_srt('wp_rem_opening_hours_opens_at'); ?> <?php echo date_i18n('g:i a', strtotime($opening_hours_single_day_val['opening_time'])); ?> - <?php echo date_i18n('g:i a', strtotime($opening_hours_single_day_val['closing_time'])); ?></span></a></li>
										<?php
									} else {
										?>
										<li class="<?php echo (strtoupper(date('D', current_time('timestamp', 1))) == strtoupper($opening_hours_single_day_var) ? 'today' : ''); ?>"><a href="javascript:void(0)"><span class="opend-day"><?php echo strtoupper(esc_html__($opening_hours_single_day_var, 'wp-rem')) ?></span> <span class="close-day"><small>:</small><?php echo wp_rem_plugin_text_srt('wp_rem_opening_hours_closed'); ?></span></a></li>
											<?php
										}
									}
									?>
