...
if (!class_exists('Wp_rem_Plugin_Functions')) {
	...
	public function wp_rem_save_post_option($post_id = '') {
	    ...
	    $wp_rem_property_old_claim_status = get_post_meta($post_id, 'wp_rem_claim_action', true);
	    $wp_rem_property_new_claim_status = (isset($_POST['wp_rem_claim_action']) ? $_POST['wp_rem_claim_action'] : '');


	    $wp_rem_property_type_status = get_post_meta($post_id, 'wp_rem_claim_type', true);
	    if (($wp_rem_property_old_claim_status == 'pending') && ($wp_rem_property_new_claim_status == 'resolved' ) && ($wp_rem_property_type_status == 'claim')) {
		// Property claim status
		do_action('wp_rem_claim_status_email', $user_data, $post_id);
	    }
	    if (($wp_rem_property_old_claim_status == 'pending') && ($wp_rem_property_new_claim_status == 'resolved' ) && ($wp_rem_property_type_status == 'flag')) {
		// Property flag status
		do_action('wp_rem_flag_status_email', $user_data, $post_id);
	    }
	    //Property Status change email send
	    $wp_rem_property_old_status = get_post_meta($post_id, 'wp_rem_property_status', true);
	    $wp_rem_property_new_status = (isset($_POST['wp_rem_property_status']) ? $_POST['wp_rem_property_status'] : '');

	    if (($wp_rem_property_old_status == 'awaiting-activation') && ($wp_rem_property_new_status == 'active' )) {
		// Property  approved
		do_action('wp_rem_property_approved_email', $user_data, $post_id);
	    } if (($wp_rem_property_old_status == 'active') && ($wp_rem_property_new_status == 'inactive')) {
		// Property not approved
		do_action('wp_rem_property_not_approved_email', $user_data, $post_id);
	    }
	    $wp_rem_data = array();
