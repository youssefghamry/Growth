...
if ( ! class_exists('Wp_rem_Post_Property_Types') ) {
    ...
    class Wp_rem_Post_Property_Types {
        ...
        public function __construct() {
            ...
            add_action('wp_rem_plugin_db_structure_updater', array( $this, 'wp_rem_plugin_db_structure_updater_callback' ), 10);
        }

        public function wp_rem_plugin_db_structure_updater_callback() {
            global $wp_rem_property_meta;
            $post_type_updated = get_option('wp_rem_post_type_updated');

            if ( $post_type_updated != 'yes' ) {
                $property_type_ids = get_posts(array(
                    'posts_per_page' => '-1',
                    'fields' => 'ids',
                    'post_type' => 'property-type',
                ));
                if ( is_array($property_type_ids) && ! empty($property_type_ids) ) {
                    foreach ( $property_type_ids as $property_type_id ) {
                        $wp_rem_features_element = get_post_meta($property_type_id, 'wp_rem_features_element', true);
                        if ( empty($wp_rem_features_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_features_element', 'on');
                        }

                        $wp_rem_tags_element = get_post_meta($property_type_id, 'wp_rem_tags_element', true);
                        if ( empty($wp_rem_tags_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_tags_element', 'on');
                        }

                        $wp_rem_property_type_open_house = get_post_meta($property_type_id, 'wp_rem_property_type_open_house', true);
                        if ( empty($wp_rem_property_type_open_house) ) {
                            update_post_meta($property_type_id, 'wp_rem_property_type_open_house', 'on');
                        }

                        $wp_rem_image_gallery_element = get_post_meta($property_type_id, 'wp_rem_image_gallery_element', true);
                        if ( empty($wp_rem_image_gallery_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_image_gallery_element', 'on');
                        }

                        $wp_rem_social_share_element = get_post_meta($property_type_id, 'wp_rem_social_share_element', true);
                        if ( empty($wp_rem_social_share_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_social_share_element', 'on');
                        }

                        $wp_rem_print_switch = get_post_meta($property_type_id, 'wp_rem_print_switch', true);
                        if ( empty($wp_rem_print_switch) ) {
                            update_post_meta($property_type_id, 'wp_rem_print_switch', 'on');
                        }

                        $wp_rem_claim_switch = get_post_meta($property_type_id, 'wp_rem_claim_switch', true);
                        if ( empty($wp_rem_claim_switch) ) {
                            update_post_meta($property_type_id, 'wp_rem_claim_switch', 'on');
                        }

                        $wp_rem_flag_switch = get_post_meta($property_type_id, 'wp_rem_flag_switch', true);
                        if ( empty($wp_rem_flag_switch) ) {
                            update_post_meta($property_type_id, 'wp_rem_flag_switch', 'on');
                        }

                        $wp_rem_location_element = get_post_meta($property_type_id, 'wp_rem_location_element', true);
                        if ( empty($wp_rem_location_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_location_element', 'on');
                        }

                        $wp_rem_attachments_options_element = get_post_meta($property_type_id, 'wp_rem_attachments_options_element', true);
                        if ( empty($wp_rem_attachments_options_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_attachments_options_element', 'on');
                        }

                        $wp_rem_video_element = get_post_meta($property_type_id, 'wp_rem_video_element', true);
                        if ( empty($wp_rem_video_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_video_element', 'on');
                        }

                        $wp_rem_virtual_tour_element = get_post_meta($property_type_id, 'wp_rem_virtual_tour_element', true);
                        if ( empty($wp_rem_virtual_tour_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_virtual_tour_element', 'on');
                        }

                        $wp_rem_faqs_options_element = get_post_meta($property_type_id, 'wp_rem_faqs_options_element', true);
                        if ( empty($wp_rem_faqs_options_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_faqs_options_element', 'on');
                        }

                        $wp_rem_near_by_options_element = get_post_meta($property_type_id, 'wp_rem_near_by_options_element', true);
                        if ( empty($wp_rem_near_by_options_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_near_by_options_element', 'on');
                        }

                        $wp_rem_yelp_places_element = get_post_meta($property_type_id, 'wp_rem_yelp_places_element', true);
                        if ( empty($wp_rem_yelp_places_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_yelp_places_element', 'on');
                        }

                        $wp_rem_walkscores_options_element = get_post_meta($property_type_id, 'wp_rem_walkscores_options_element', true);
                        if ( empty($wp_rem_walkscores_options_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_walkscores_options_element', 'on');
                        }

                        $wp_rem_floor_plans_options_element = get_post_meta($property_type_id, 'wp_rem_floor_plans_options_element', true);
                        if ( empty($wp_rem_floor_plans_options_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_floor_plans_options_element', 'on');
                        }

                        $wp_rem_appartments_options_element = get_post_meta($property_type_id, 'wp_rem_appartments_options_element', true);
                        if ( empty($wp_rem_appartments_options_element) ) {
                            update_post_meta($property_type_id, 'wp_rem_appartments_options_element', 'on');
                        }
                    }
                }
                update_option('wp_rem_post_type_updated', 'yes');
            }
        }
