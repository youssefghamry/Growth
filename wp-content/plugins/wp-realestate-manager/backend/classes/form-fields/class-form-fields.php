...
if ( ! class_exists('wp_rem_form_fields') ) {
	...
	class wp_rem_form_fields {
		 ...
		 * @ render label
		 */
		public function wp_rem_form_text_render($params = array()) {
			...
			if ( $pagenow == 'post.php' ) {
				...
				if ( isset($cus_field) && $cus_field == true ) {
					$wp_rem_value = get_the_author_meta($id, isset($user->ID) ? $user->ID : '');
				} else {
					if ( isset($id) && $id != '' ) {
						$wp_rem_value = get_the_author_meta('wp_rem_' . $id, isset($user->ID) ? $user->ID : '');
					}
				}
			} else {
