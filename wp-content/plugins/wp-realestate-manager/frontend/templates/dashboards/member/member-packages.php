                ...
                <?php if ( isset($pkg_query) && $pkg_query != '' && $pkg_query->have_posts() ) : ?>
                    <div class="all-pckgs-sec">
                        ...
                        while ( $pkg_query->have_posts() ) : $pkg_query->the_post();
                            ...
                            $pkg_expire_date = date_i18n(get_option('date_format'), $transaction_expiry_date);
                            $wp_rem_property_idss = get_post_meta(get_the_ID(), 'wp_rem_property_ids', true);

                            if ( empty($wp_rem_property_idss) ) {
                                $wp_rem_property_usedd = 0;
                            } else {
                                $wp_rem_property_usedd = absint(sizeof($wp_rem_property_idss));
                            }
                            ...
                            <div class="wp-rem-pkg-holder">
                                <div class="wp-rem-pkg-header">
                                    <div class="pkg-title-price pull-left">
                                        ...
                                        <span class="pkg-price"><strong><?php echo wp_rem_plugin_text_srt('wp_rem_member_price_s');  ?></strong> <span> : <?php echo wp_rem_get_order_currency($package_price, $wp_rem_currency_sign, $currency_position);?></span> </span>
                                        <span class="pkg-expiry"><strong><?php echo wp_rem_plugin_text_srt('wp_rem_member_expiry_date'); ?> </strong><span> : <?php echo esc_html($pkg_expire_date); ?></span></span>
                                        <span class="pkg-properties"><strong><?php echo wp_rem_plugin_text_srt('wp_rem_member_properties'); ?></strong> <span> : <?php echo esc_html($wp_rem_property_usedd); ?>/<?php echo esc_html($transaction_properties); ?></span></span>
                                    </div>
