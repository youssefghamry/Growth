...
add_filter('wp_authenticate_user', 'wp_rem_auth_login', 10, 2);

function wp_rem_auth_login($user = '', $password = '') {
    ...
    if ( $user_status == 0 ) {
        return new WP_Error('user_status_error', wp_rem_plugin_text_srt('wp_rem_login_not_activated'));
    } else {
        return $user;
    }
}
    ...
    function ajax_login() {
        ...
        if ( $credentials['user_login'] == '' ) {
            ...
            if ( is_wp_error($status) ) {
                ...
                $wp_rem_redirect_url = '';
                if ( $wp_rem_page_id != '' ) {
                    $wp_rem_redirect_url = wp_rem_wpml_lang_page_permalink($wp_rem_page_id, 'page');
                } else {
    ...
    function wp_rem_registration_validation($atts = '', $given_params = '') {
        ...
        if ( $given_params == '' ) {
            if ( empty($username) ) {
                ...
                echo json_encode($json);
                exit();
            } elseif ( username_exists($username) ) { // username already exists
                $json['type'] = "error";
                $json['msg'] = wp_rem_plugin_text_srt('wp_rem_username_alraedy_exists');
                echo json_encode($json);
                exit();
            }
        ...
        if ( username_exists($username) ) { // username already exists
            $json['type'] = "error";
            $json['msg'] = wp_rem_plugin_text_srt('wp_rem_username_alraedy_exists');
            echo json_encode($json);
            exit();
        ...
        if ( is_wp_error($status) ) {
             ...
             */
            $company_phone = wp_rem_get_input('wp_rem_phone_no' . $id, NULL, '');
            $company_data = array(
            ...
            if ( $company_ID ) {
                ...
                do_action('wp_rem_assign_free_package_to_member', $company_ID);
            }
