...
<aside class="filters-sidebar col-lg-3 col-md-3 col-sm-12 col-xs-12">
	...
	if ($filter_search_box == 'yes' && $search_box_sidebar == 'yes') {
		...
		<?php
		echo wp_rem_wpml_lang_code_field('text');
		...
		if ($property_open_house_filter == 'yes') {
			...
			$price_min = wp_rem_property_price_options($wp_rem_price_minimum_options, $wp_rem_price_max_options, $wp_rem_price_interval, wp_rem_plugin_text_srt('wp_rem_property_leftflter_min'));
			$price_max = wp_rem_property_price_options($wp_rem_price_minimum_options, $wp_rem_price_max_options, $wp_rem_price_interval, wp_rem_plugin_text_srt('wp_rem_property_leftflter_max'));
		...
		if (isset($property_type) && $property_type != '') {
			...
			foreach ($wp_rem_property_type_cus_fields as $cus_fieldvar => $cus_field) {
				...
				?> 
		    	    <div class="select-categories">
		    		<h6> <?php
				    if ($cus_field['type'] == 'dropdown' || $cus_field['type'] == 'range') {
					...
					if (isset($cus_field['fontawsome_icon']) && $cus_field['fontawsome_icon'] != '') {
					    ...
					    ?>
			    		<span class="rm-side-icons"><i class="<?php echo esc_html($cus_field['fontawsome_icon']); ?>"></i></span>
					<?php
					}
				    }
				    ?><?php echo esc_html($cus_field['label']); ?></h6>
					<?php
