...
function wp_rem_enquiry_detail(enquiry_id, type, read_status) {
    "use strict";
...
function wp_rem_arrange_viewing_detail(viewing_id, type, read_status) {
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            if( type == 'my' && read_status == '0'){
                var count = jQuery('.dashboard-nav').find('.user_dashboard_ajax').find('b.count-viewings').html();
                if( typeof count !== "undefined" && count > 0 ){
                    count -= 1;
                    jQuery('.dashboard-nav').find('.user_dashboard_ajax').find('b.count-viewings').html(count);
                }
            }
            wp_rem_hide_loader('.loader-holder');
            jQuery('.user-holder').html(response);
        ...
        success: function (response) {
            ...
            if (response.read_type == 'read') {
                if (typeof count !== "undefined"){
                    var sum = 1;
                    var new_val = parseInt(count) + parseInt(sum);
                    jQuery('.dashboard-nav').find('.user_dashboard_ajax').find('b.count-viewings').html(new_val);
                }
