...
if ( ! class_exists('Wp_rem_Email') ) {
    ...
    class Wp_rem_Email {
        ...
        public function __construct() {
            ...
            add_action('process_emails', array( $this, 'process_emails_callback' ), 20,1);
            //add_action('wp_ajax_nopriv_process_emails', array( $this, 'process_emails_callback' ), 20,1);
        ...
        public function process_emails_callback($post_id = '') {
           ...
           // $post_id = isset($_REQUEST['email_id']) ? $_REQUEST['email_id'] : 0;
            ...
            if ( $query->have_posts() ) {
                while ( $query->have_posts() ) {
                    ...
                    if ( $wp_rem_email_logs != 'on' && get_post_type($wp_rem_post_id) == $this->email_post_type_name ) {
                        wp_delete_post($wp_rem_post_id);
                    }
                }
                wp_reset_postdata();
            }
        ...
        public function phpmailer_init_callback($phpmailer) {
            ...
            $options = apply_filters('wp_rem_translate_options', $options);
        ...
        public function send_smtp_mail_callback() {
            ...
            $message .= "\n\n";
            $wp_rem_from_name = isset($options['wp_rem_sender_name']) && $options['wp_rem_sender_name'] != '' ? $options['wp_rem_sender_name'] : get_bloginfo('name');
            $wp_rem_from_email = isset($options['wp_rem_smtp_sender_email']) && $options['wp_rem_smtp_sender_email'] != '' ? $options['wp_rem_smtp_sender_email'] : get_option('admin_email');
            $headers = array();
            if ( $wp_rem_from_email != '' && $wp_rem_from_name != '' ) {
                $headers[] = "From:" . $wp_rem_from_name . " <" . $wp_rem_from_email . ">";
            } else if ( $wp_rem_from_email != '' && $wp_rem_from_name == '' ) {
                $headers[] = "From:" . $wp_rem_from_email;
            }
        ...
        public function send_mail_callback($args) {
            ...
            if ( $post_id != 0 ) {
                //wp_remote_get(admin_url('admin-ajax.php?action=process_emails&email_id=' . $post_id), array( 'timeout' => 0, 'httpversion' => '1.1' ));
                do_action('process_emails',$post_id);
            }
            if ( $class_obj != '' ) {
