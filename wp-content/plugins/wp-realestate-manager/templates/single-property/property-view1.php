 ...
 *
 */
global $post, $wp_rem_plugin_options, $wp_rem_theme_options, $wp_rem_post_property_types, $wp_rem_author_info;
$post_id = $post->ID;
    ...
    <div class="page-section property-detail-view1-section">
        <div class="property-detail">
			...
			?>
				...
				<?php do_action('wp_rem_enquire_arrange_buttons_element_html', $post_id, 'view-1'); ?>
    			</div>
				...
				if (count($floor_plans) > 0) :
	    			...
	    			<div class="architecture-holder" id="floor-plans">
						...
						<div <?php echo ($floor_id); ?> class="tab-pane fade in <?php echo esc_html($active); ?>">
						    <h5><?php echo esc_html($floor_plan['floor_plan_title']); ?></h5>
	    ...
	    <?php endforeach; ?>
			    ...
			    <?php } ?>
			    <?php do_action('wp_rem_author_info_html', $post_id, 'view-1'); ?>
    <?php do_action('wp_rem_reviews_ui', $post_id) ?>
    		    </div>
    		    <div class="sidebar col-lg-4 col-md-4 col-sm-12 col-xs-12">
			    <?php do_action('wp_rem_sidebar_gallery_html', $post_id, 'detail_view1'); ?>
			    <?php do_action('wp_rem_sidebar_map_html', $post_id, 'detail_view1'); ?>
			    <?php
			    $active_plugin = false;
			    $active_plugin = apply_filters('wp_rem_qwad_plugin_active', $active_plugin);
			    if ($active_plugin) {
				do_action('wp_rem_sedebar_enquiry_form', $post_id, 'detail_view1');
			    } else {
				do_action('wp_rem_sidebar_contact_html', $post_id, 'detail_view1');
			    }
			    ?>
			    <?php do_action('wp_rem_sidebar_member_info_html', $post_id, 'detail_view1'); ?>
			<?php do_action('wp_rem_payment_calculator_html', $post_id, 'detail_view1'); ?>
    		    </div>
			<?php
		    } else {
			?> 
