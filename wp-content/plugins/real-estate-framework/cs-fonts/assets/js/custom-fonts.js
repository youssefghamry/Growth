...
jQuery(document).ready(function() {
    jQuery('body').on('click', '.export-btn a.export-fonts-btn', function () {
        var button = jQuery(this);
        jQuery.ajax({
            type: "POST",
            url: ajaxurl,
            data: 'action=wp_rem_export_fonts',
            dataType: 'json',
            success: function (response) {
                if( response.type === 'success' ){
                    if( response.url !== '' ){
                        jQuery('.export-btn a.export-fonts').prop('href', response.url);
                        jQuery('.export-btn a.export-fonts').attr('download', response.name);
                        jQuery('#export-fonts').get(0).click();
                    }
                }
            }
        });
