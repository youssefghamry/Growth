...
if ( ! class_exists('WP_REM_PRE_BANK_TRANSFER') ) {
    ...
    class WP_REM_PRE_BANK_TRANSFER {
        ...
        public function __construct() {
            ...
            $wp_rem_gateway_options = apply_filters('wp_rem_translate_options', $wp_rem_gateway_options );
        }
        ...
        public function wp_rem_proress_request($params = '') {
            ...
            if ( isset($transaction_package) && $transaction_package <> '' ) {
                ...
                $wp_rem_detail .= '<li>' .  wp_rem_plugin_text_srt('wp_rem_banktransfer_packages'). $transaction_package_title . ' - '.wp_rem_get_currency($transaction_package_price,true) . '</li>';
                ...
                $wp_rem_detail .= '<li>' . wp_rem_plugin_text_srt('wp_rem_banktransfer_charges'). wp_rem_get_currency($wp_rem_totl_amount,true) . '</li>';
            }
            ...
            if ( isset($vat_amount) && $vat_amount > 0 && $wp_rem_totl_amount > 0 ) {
                ...
                $wp_rem_totl_amount = WP_REM_FUNCTIONS()->num_format($wp_rem_totl_amount);
                $wp_rem_detail .= '<li>' .  wp_rem_plugin_text_srt('wp_rem_banktransfer_vat'). ' ' . wp_rem_get_currency($vat_amount, true) . '</li>';
                $wp_rem_detail .= '<li>' .  wp_rem_plugin_text_srt('wp_rem_banktransfer_total_charge').  wp_rem_get_currency($wp_rem_totl_amount, true) . '</li>';
            }
