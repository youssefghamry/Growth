...
if(!class_exists('wp_rem_google_fonts_admin')){
	class wp_rem_google_fonts_admin{
		function __construct(){
			...
			add_action( 'wp_ajax_wp_rem_export_fonts', array($this, 'wp_rem_export_fonts_callback') );
		...
		function wp_rem_add_selected_custom_font(){
			...
			$wp_rem_selected_custom_fonts = get_option('wp_rem_selected_custom_fonts');
			if( empty($wp_rem_selected_custom_fonts)){
				$wp_rem_selected_custom_fonts = array();
			}
			
			$wp_rem_selected_custom_fonts['name'][] = $font_name;
			$wp_rem_selected_custom_fonts['woff'][] = $woff_font;
			$wp_rem_selected_custom_fonts['ttf'][] = $ttf_font;
			$wp_rem_selected_custom_fonts['svg'][] = $svg_font;
			$wp_rem_selected_custom_fonts['eot'][] = $eot_font;
			update_option('wp_rem_selected_custom_fonts', $wp_rem_selected_custom_fonts);
			$last_key = end(array_keys($wp_rem_selected_custom_fonts['name']));
		...
		function wp_rem_delete_selected_custom_font(){
			...
			unset($wp_rem_selected_custom_fonts['name'][$font_key]);
			unset($wp_rem_selected_custom_fonts['woff'][$font_key]);
			unset($wp_rem_selected_custom_fonts['ttf'][$font_key]);
			unset($wp_rem_selected_custom_fonts['svg'][$font_key]);
			unset($wp_rem_selected_custom_fonts['eot'][$font_key]);
			
			$wp_rem_selected_custom_fonts['name'] = array_values($wp_rem_selected_custom_fonts['name']);
			$wp_rem_selected_custom_fonts['woff'] = array_values($wp_rem_selected_custom_fonts['woff']);
			$wp_rem_selected_custom_fonts['ttf'] = array_values($wp_rem_selected_custom_fonts['ttf']);
			$wp_rem_selected_custom_fonts['svg'] = array_values($wp_rem_selected_custom_fonts['svg']);
			$wp_rem_selected_custom_fonts['eot'] = array_values($wp_rem_selected_custom_fonts['eot']);
		...
		public function wp_rem_selected_custom_fonts_list() {
			$wp_rem_selected_custom_fonts = get_option('wp_rem_selected_custom_fonts');
			$custom_fonts = array();
			if ( is_array($wp_rem_selected_custom_fonts) && !empty($wp_rem_selected_custom_fonts['name'])) {
				foreach ( $wp_rem_selected_custom_fonts['name'] as $key => $wp_rem_custom_font  ) {
					$font_name = isset($wp_rem_selected_custom_fonts['name'][$key]) ? $wp_rem_selected_custom_fonts['name'][$key] : '';
					if( $font_name != '' ){
		...
		public function wp_rem_export_fonts_callback() {
			...
			$wp_rem_selected_custom_fonts = get_option('wp_rem_selected_custom_fonts');
			$wp_rem_fonts = array();
			$wp_rem_fonts['wp_rem_google_fonts'] = $wp_rem_google_fonts;
			$wp_rem_fonts['wp_rem_added_google_fonts'] = $added_google_fonts;
			$wp_rem_fonts['wp_rem_custom_fonts_list'] = $wp_rem_custom_fonts_list;
			$wp_rem_fonts['wp_rem_selected_custom_fonts'] = $wp_rem_selected_custom_fonts;
			
			$wp_rem_fonts_fields = json_encode($wp_rem_fonts, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);
			
			$wp_rem_upload_dir = FRAMEWORK_CORE_DIR . '/assets/backups/';
			$name = 'fonts.json';
			$wp_rem_filename = trailingslashit($wp_rem_upload_dir) . $name;
			$wp_rem_fileurl = FRAMEWORK_PLUGIN_URL. '/assets/backups/'. $name;
			if ( ! $wp_filesystem->put_contents($wp_rem_filename, $wp_rem_fonts_fields, FS_CHMOD_FILE) ) {
				echo json_encode( array( 'type' => 'error', 'name' => $name, 'url' => $wp_rem_fileurl ) );
			} else {
				echo json_encode( array( 'type' => 'success', 'name' => $name, 'url' => $wp_rem_fileurl ));
			}
			die();
		}
