    ...
    class Wp_rem_Arrange_Viewing_Detail {
            ...
            while ( $order_query->have_posts() ): $order_query->the_post();
                ...
                <div class="list-detail-options has-checkbox">
                    ...
                    if ( $type == 'received' ) {
                        ...
                        $data_param = 'data-param="' . $wp_rem_property_id . '"';
                    } else {
                    ...
                    ?>
                    <div class="element-title"><h4>
                            <?php if ( $type != 'received' ) { ?>
                                <a id="wp_rem_member_viewings" class="user_dashboard_ajax" href="javascript:void(0);"><?php echo wp_rem_plugin_text_srt('wp_rem_member_register_requested_viewings'); ?></a>&nbsp;>&nbsp;
                            <?php } ?>
                            <?php if ( $type != 'my' ) { ?>
                                <a id="<?php echo esc_html($viewing_id); ?>" <?php echo $data_param; ?> class="user_dashboard_ajax" href="javascript:void(0);"><?php echo get_the_title($wp_rem_property_id); ?></a>&nbsp;>&nbsp;
                            <?php } ?>
                            <?php if ( $type == 'received' ) { ?>
                                <?php echo wp_rem_plugin_text_srt('wp_rem_member_received_viewings'); ?>
                            <?php } else { ?>
                                <?php echo wp_rem_plugin_text_srt('wp_rem_member_register_requested_viewings'); ?>
                            <?php } ?> #<?php echo absint($post_id); ?>
                        </h4></div>
                    ...
                    <div class="order-read-unread">
                        <div class="input-field">
                            <?php
                            $read = wp_rem_plugin_text_srt('wp_rem_viewing_detail_mark_read');
                            $unread = wp_rem_plugin_text_srt('wp_rem_viewing_detail_mark_unread');
                            ?>
                ...
                <div class="description-holder">
                    ...
                    <div class="order-detail-holder">
                        <strong class="heading">
                            <?php if ( $type == 'received' ) { ?>
                                <?php echo wp_rem_plugin_text_srt('wp_rem_member_register_received_viewing_detail'); ?>
                            <?php } else { ?>
                                <?php echo wp_rem_plugin_text_srt('wp_rem_member_register_requested_viewing_detail'); ?>
                            <?php } ?>
                        </strong>
                        <?php
        ...
        public function arrange_viewing_status_unread_ui($post_id = '') {
            ...
            <div class="order-status-read-unread-holder">
                <?php if ( $property_member == $current_login_member ) { ?>
                    <strong class="heading">
                        <?php
                        echo wp_rem_plugin_text_srt('wp_rem_viewing_detail_status');
                        ?>
                    </strong>
                    <div class="input-field">
                        <?php
                ...
                <?php } else if ( $viewing_member == $current_login_member && $viewing_status == 'Completed' ) { ?>
                    <div class="order-status-process viewing-status">
                        <p><?php echo wp_rem_plugin_text_srt('wp_rem_viewing_detail_viewing_completed') ?> <a href="javascript:void(0);" onclick="wp_rem_closed_arrange_viewing('<?php echo esc_html($post_id); ?>');"><?php echo wp_rem_plugin_text_srt('wp_rem_viewing_detail_closed_viewing'); ?></a></p>
                    </div>
                <?php } else if ( $viewing_member == $current_login_member ) { ?>
                    <div class="order-status-process viewing-status">
                        <p><?php echo wp_rem_plugin_text_srt('wp_rem_viewing_detail_viewing_is') . $viewing_status; ?></p>
                    </div>
                <?php } ?>
            </div>
            <?php
        ...
        public function wp_rem_update_viewing_read_status_callback() {
            ...
            if ( $viewing_id ) {
                ...
                if ( $viewing_read_status == 0 ) {
                    $json['read_type'] = 'read';
                    $json['type'] = 'success';
                    $json['msg'] = wp_rem_plugin_text_srt('wp_rem_viewing_unread');
                } else {
                    $json['read_type'] = 'unread';
                    $json['type'] = 'success';
                    $json['msg'] = wp_rem_plugin_text_srt('wp_rem_viewing_read');
                }
            }
        ...
        public function wp_rem_update_viewing_status_callback() {
            ...
            if ( $viewing_id && $viewing_status ) {
                ...
                /* Adding Notification */
                $property_id = get_post_meta($viewing_id, 'wp_rem_property_id', $viewing_status);
                $property_member = get_post_meta($viewing_id, 'wp_rem_property_member', $viewing_status);
                $viewing_member = get_post_meta($viewing_id, 'wp_rem_viewing_member', $viewing_status);
                $member_name = '<a href="' . esc_url(get_the_permalink($property_member)) . '">' . esc_html(get_the_title($property_member)) . '</a>';
                $notification_array = array(
                    'type' => 'viewing_status',
                    'element_id' => $property_id,
                    'sender_id' => $property_member,
                    'reciever_id' => $viewing_member,
                    'message' => force_balance_tags($member_name . ' ' . strtolower($viewing_status) . ' ' . wp_rem_plugin_text_srt('wp_rem_notification_update_viewing_status') . ' <a href="javascript:void();" onclick="javascript:wp_rem_arrange_viewing_detail(\'' . $viewing_id . '\',\'my\');">' . wp_trim_words(get_the_title($viewing_id), 3) . '</a>'),
                );
                ...
                $json['type'] = "success";
                $json['msg'] = wp_rem_plugin_text_srt('wp_rem_viewing_status_changed');
            } else {
                $json['type'] = "error";
                $json['msg'] = wp_rem_plugin_text_srt('wp_rem_viewing_status_changed');
            }
        ...
        public function wp_rem_closed_viewing_callback() {
            ...
            $json['type'] = "success";
            $json['msg'] = wp_rem_plugin_text_srt('wp_rem_viewing_status_closed');
            echo json_encode($json);
            exit();
        }
