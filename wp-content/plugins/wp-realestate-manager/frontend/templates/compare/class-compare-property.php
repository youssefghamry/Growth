...
if ( ! class_exists('wp_rem_compare_properties') ) {
    ...
    class wp_rem_compare_properties {
        ...
        public function __construct() {
			...
			add_action('wp_ajax_wp_rem_removed_compare', array( $this, 'wp_rem_removed_compare' ));
            add_action('wp_ajax_nopriv_wp_rem_removed_compare', array( $this, 'wp_rem_removed_compare' ));
			...
			add_action('wp_ajax_wp_rem_clear_compare_list', array( $this, 'wp_rem_clear_compare_list_callback' ));
            add_action('wp_ajax_nopriv_wp_rem_clear_compare_list', array( $this, 'wp_rem_clear_compare_list_callback' ));

            add_action('wp_rem_compare_btn', array( $this, 'wp_rem_property_compare_button' ), 10, 3);
            add_action('wp_rem_property_compare', array( $this, 'wp_rem_property_compare_frontend_ui' ), 10, 5);

            add_filter('wp_rem_is_compare', array( $this, 'wp_rem_property_is_compare' ), 10, 2);
            add_action('wp_rem_detail_compare_btn', array( $this, 'wp_rem_property_detail_compare_button' ), 10, 3);
			add_action('wp_rem_property_compare_sidebar', array( $this, 'wp_rem_property_compare_sidebar_ui' ));
		...
		public function wp_rem_clear_compare_list_callback() {
            setcookie('wp_rem_compare_list', '', time() + 86400, '/');
			echo json_encode(array( 'type' => 'success' ));
            die;
        }
        ...
        public function wp_rem_compare_add() {
                ...
                if ( $wp_rem_check_action == 'check' ) {
                    ...
                    if ( $already_in_list == true ) {
                        ...
                        if ( $compare_listing_url != '' ) {
                            ...
                            $added_succesfully_msg .= wp_rem_plugin_text_srt('wp_rem_compare_added_to_compare');
                    ...
                    $mark_msg = '';
                    $mark_msg .= wp_rem_plugin_text_srt('wp_rem_compare_was_removed_from_compare');
                    if ( $compare_listing_url != '' ) {
                        $mark_msg .= $before_btn;
                        $mark_msg .= $after_btn;
                    }
			...
			ob_start();
				$this->wp_rem_property_compare_sidebar_list_render( $wp_rem_prop_id, $property_type_id );
		...
		public function wp_rem_removed_compare() {
            ...
            echo json_encode(array( 'type' => 'success', 'mark' => wp_rem_plugin_text_srt('wp_rem_compare_was_removed_from_compare') ));
            die;
        }
		
		public function wp_rem_property_compare_sidebar_ui() {
			global $wp_rem_plugin_options;
			
			if( did_action('wp_rem_property_compare_sidebar') == 1 ){
				$compare_listing_url = isset($wp_rem_plugin_options['wp_rem_compare_list_page']) ? $wp_rem_plugin_options['wp_rem_compare_list_page'] : '';
				$cookie_compare_list = '';
				if ( isset($_COOKIE['wp_rem_compare_list']) && $_COOKIE['wp_rem_compare_list'] != '' ) {
				...
				if( isset($cookie_compare_list) && !empty($cookie_compare_list) && is_array($cookie_compare_list) ){
					foreach( $cookie_compare_list as $key => $val ){
						if( $val['type_id'] != '' && !empty($val['list_ids']) ){
							$compare_properties_list[$val['type_id']] = $val['list_ids'];
						}
					}
				}
					...
					<div class="sidebar-panel-content">
						<div class="sidebar-panel-header">
							<strong class="sidebar-panel-title"> 
								<?php echo wp_rem_plugin_text_srt('wp_rem_sidebar_compare_properties_lable'); ?>
						...
						<div class="sidebar-panel-body">
							<div class="sidebar-properties-list">
								<ul>
									...
									if( isset($compare_properties_list) && !empty($compare_properties_list)){
										foreach( $compare_properties_list as $type_id => $property_ids ){
											if( isset($compare_properties_list) && !empty($compare_properties_list)){
												foreach( $property_ids as $property_id ){
													$this->wp_rem_property_compare_sidebar_list_render( $property_id, $type_id );
								...
								<div class="sidebar-btn-holder">
									<a href="javascript:void(0);" class="bgcolor sidebar-property-btn clear-compare-list"><?php echo wp_rem_plugin_text_srt('wp_rem_sidebar_compare_reset_button_lable'); ?></a>
									<?php if( $compare_listing_url != '') { ?>
										<a href="<?php echo wp_rem_wpml_lang_page_permalink($compare_listing_url, 'page'); ?>" class="sidebar-property-btn text-color border-color"><?php echo wp_rem_plugin_text_srt('wp_rem_sidebar_compare_button_lable'); ?></a>
									<?php } ?>
								</div>
								<div class="compare-response" style="display:none;"></div>
							</div>
							<button class="bgcolor sidebar-panel-btn" style="display:<?php echo $compare_open_close; ?>"><i class="icon-keyboard_arrow_left"></i></button>
						</div>
					</div>
				</div>
				<?php
		...
		public function wp_rem_property_compare_sidebar_list_render( $property_id = '', $type_id = '' ) {
			if( $property_id != '' ){
				...
				?>
				<li class="compare-property-<?php echo intval($property_id); ?>" data-id="<?php echo intval($property_id); ?>" data-type-id="<?php echo intval($type_id); ?>">
					<div class="property-item">
						...
						<div class="text-holder">
							<strong class="property-title"><a href="<?php echo esc_url(get_permalink($property_id)); ?>"><?php echo esc_html(get_the_title($property_id)); ?></a></strong>
        ...
        public function wp_rem_property_compare_frontend_ui($prop_id = '', $show_compare = 'no', $tooltip = 'no', $before_html = '', $after_html = '') {
            global $property_random_id, $wp_rem_plugin_options;
            $wp_rem_all_compare_buttons = isset($wp_rem_plugin_options['wp_rem_all_compare_buttons']) ? $wp_rem_plugin_options['wp_rem_all_compare_buttons'] : '';
            if ( $wp_rem_all_compare_buttons == 'on' && $show_compare == 'yes' ) {
                ...
                $prop_compare_check = '';
                $pro_compare_class = '';
                $compare_label = wp_rem_plugin_text_srt('wp_rem_compare_label');
                if ( isset($cookie_compare_list["type_{$property_type_id}"]['list_ids']) ) {
                    $wp_rem_type_comp_list = $cookie_compare_list["type_{$property_type_id}"]['list_ids'];
                    if ( is_array($wp_rem_type_comp_list) && in_array($prop_id, $wp_rem_type_comp_list) ) {
                        $prop_compare_check = ' checked="checked"';
                        $pro_compare_class = ' compared';
                        $compare_label = wp_rem_plugin_text_srt('wp_rem_compared_label');
                    }
                }
                $html = '';
                $html .= $before_html;
                $html .= '<div class="compare-properties-' . absint($prop_id) . ' compare-property ' . esc_html($pro_compare_class) . '">';
                $html .= '<input type="checkbox" ' . $prop_compare_check . ' class="wp_rem_property_compare_check" data-id="' . absint($prop_id) . '" data-random="' . absint($prop_id.$rand) . '" name="list" value="check-listn" id="check-list' . absint($prop_id.$rand) . '">';
        ...
        public function wp_rem_property_detail_compare_button($prop_id = '', $before_html = '', $after_html = '') {
            global $wp_rem_plugin_options;
            $wp_rem_all_compare_buttons = isset($wp_rem_plugin_options['wp_rem_all_compare_buttons']) ? $wp_rem_plugin_options['wp_rem_all_compare_buttons'] : '';
            if ( $wp_rem_all_compare_buttons == 'on' ) {
				...
				$html = $before_html;
					$html .= '<div class="detail-compare-btn"><a class="btn-compare wp-rem-btn-compare" data-type="detail-page" data-id="' . $prop_id . '" data-check="' . $prop_compare_check . '" data-ajaxurl="' . admin_url('admin-ajax.php') . '"><i class="icon-compare-filled2"></i><span>' . $compare_title . '</span></a></div>';
                $html .= $after_html;
				if ( isset($cookie_compare_list["type_{$property_type_id}"]['list_ids']) ) {
                    $wp_rem_type_comp_list = $cookie_compare_list["type_{$property_type_id}"]['list_ids'];
                    if ( is_array($wp_rem_type_comp_list) && in_array($prop_id, $wp_rem_type_comp_list) ) {
						...
						$html = $before_html;
                        $html .= '<div class="detail-compare-btn"><a class="btn-compare wp-rem-btn-compare" data-type="detail-page" data-id="' . $prop_id . '" data-check="' . $prop_compare_uncheck . '" data-check="' . $prop_compare_check . '" data-ajaxurl="' . admin_url('admin-ajax.php') . '"><i class="icon-compare-filled2"></i><span>' . $remove_compare_title . '</span></a></div>';
						$html .= $after_html;
					}
                }
				echo force_balance_tags($html);
        ...
        public function wp_rem_compare_properties_listing($atts, $content = "") {
            ...
            $defaults = array(
                ...
                'compare_subtitle' => '',
                'compare_title_align' => '',
                'wp_rem_compare_element_title_color' => '',
                'wp_rem_compare_element_subtitle_color' => '',
                'wp_rem_compare_seperator_style' => '',
            );
            ...
            <div class="row">
                <div class="section-fullwidth col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    $wp_rem_element_structure .= wp_rem_plugin_title_sub_align($property_compare_title, $compare_subtitle, $compare_title_align, $wp_rem_compare_element_title_color, $wp_rem_compare_seperator_style, $wp_rem_compare_element_subtitle_color);
                        ...
                        <?php if ( is_array($meta_post_ids_arr) && sizeof($meta_post_ids_arr) > 1 ) {
                            ...
                            <ul class="wp-rem-compare-list">
                                ...
                                <li>
                                    ...
                                    <?php foreach ( $meta_post_ids_arr as $property_id ) { ?>
                                        <div class="wp-rem-compare-box dev-rem-<?php echo absint($property_id) ?>">
                                            <span>
                                                <?php
                                ...
                                <li>
                                    <div class="compare-text-div">
                                        ...
                                        if ( $wp_rem_search_result_page != '' && is_numeric($wp_rem_search_result_page) ) {
                                            $listing_url = '<a href="' . wp_rem_wpml_lang_page_permalink($wp_rem_search_result_page, 'page') . '">' . ' ' . wp_rem_plugin_text_srt('wp_rem_compare_click_here') . '</a>';
                                        }
                        ...
                        <div class="cs-pbwp-content">
                            <div class="cs-wrapp-clone cs-shortcode-wrapp">
                                ...
                                $wp_rem_opt_array = array(
                                    ...
                                    'echo' => true,
                                    'field_params' => array(
                                        'std' => $wp_rem_compare_element_title_color,
                                        'cust_name' => 'wp_rem_compare_element_title_color[]',
                                        'classes' => 'bg_color',
                                        'return' => true,
                                    ),
                                );
                                ...
                                $wp_rem_opt_array = array(
                                    ...
                                    'echo' => true,
                                    'field_params' => array(
                                        'std' => $wp_rem_compare_element_subtitle_color,
                                        'cust_name' => 'wp_rem_compare_element_subtitle_color[]',
                                        'classes' => 'bg_color',
                                        'return' => true,
                                    ),
                                );
                                ...
                                $wp_rem_opt_array = array(
                                    ...
                                    'echo' => true,
                                    'field_params' => array(
                                        'std' => esc_attr($wp_rem_compare_seperator_style),
                                        'classes' => 'chosen-select-no-single',
                                        'cust_name' => 'wp_rem_compare_seperator_style[]',
                                        'return' => true,
                                        'options' => array(
        ...
        public function wp_rem_cs_save_page_builder_data_wp_rem_compare_property_callback($args) {
            ...
            if ( $widget_type == "wp_rem_compare_property" || $widget_type == "cs_wp_rem_compare_property" ) {
                ...
                if ( isset($data['wp_rem_cs_widget_element_num'][$counters['wp_rem_cs_counter']]) && $data['wp_rem_cs_widget_element_num'][$counters['wp_rem_cs_counter']] == 'shortcode' ) {
                    ...
                    if ( isset($data['compare_subtitle'][$counters['wp_rem_cs_counter_wp_rem_compare_property']]) && $data['compare_subtitle'][$counters['wp_rem_cs_counter_wp_rem_compare_property']] != '' ) {
                        $wp_rem_cs_bareber_wp_rem_compare_property .= 'compare_subtitle="' . htmlspecialchars($data['compare_subtitle'][$counters['wp_rem_cs_counter_wp_rem_compare_property']], ENT_QUOTES) . '" ';
                    }
                    if ( isset($data['compare_title_align'][$counters['wp_rem_cs_counter_wp_rem_compare_property']]) && $data['compare_title_align'][$counters['wp_rem_cs_counter_wp_rem_compare_property']] != '' ) {
                        $wp_rem_cs_bareber_wp_rem_compare_property .= 'compare_title_align="' . htmlspecialchars($data['compare_title_align'][$counters['wp_rem_cs_counter_wp_rem_compare_property']], ENT_QUOTES) . '" ';
                    }
                    if ( isset($data['wp_rem_compare_element_title_color'][$counters['wp_rem_cs_counter_wp_rem_compare_property']]) && $data['wp_rem_compare_element_title_color'][$counters['wp_rem_cs_counter_wp_rem_compare_property']] != '' ) {
                        $wp_rem_cs_bareber_wp_rem_compare_property .= 'wp_rem_compare_element_title_color="' . htmlspecialchars($data['wp_rem_compare_element_title_color'][$counters['wp_rem_cs_counter_wp_rem_compare_property']], ENT_QUOTES) . '" ';
                    }
                    if ( isset($data['wp_rem_compare_element_subtitle_color'][$counters['wp_rem_cs_counter_wp_rem_compare_property']]) && $data['wp_rem_compare_element_subtitle_color'][$counters['wp_rem_cs_counter_wp_rem_compare_property']] != '' ) {
                        $wp_rem_cs_bareber_wp_rem_compare_property .= 'wp_rem_compare_element_subtitle_color="' . htmlspecialchars($data['wp_rem_compare_element_subtitle_color'][$counters['wp_rem_cs_counter_wp_rem_compare_property']], ENT_QUOTES) . '" ';
                    }
                    if ( isset($data['wp_rem_compare_seperator_style'][$counters['wp_rem_cs_counter_wp_rem_compare_property']]) && $data['wp_rem_compare_seperator_style'][$counters['wp_rem_cs_counter_wp_rem_compare_property']] != '' ) {
                        $wp_rem_cs_bareber_wp_rem_compare_property .= 'wp_rem_compare_seperator_style="' . htmlspecialchars($data['wp_rem_compare_seperator_style'][$counters['wp_rem_cs_counter_wp_rem_compare_property']], ENT_QUOTES) . '" ';
                    }
                    $wp_rem_cs_bareber_wp_rem_compare_property .= ']';
