...
if ( ! function_exists('wp_rem_cs_locations_shortcode') ) {
    ...
    function wp_rem_cs_locations_shortcode($atts) {
        ...
        if ( function_exists('wp_rem_cs_var_page_builder_element_sizes') ) {
            $html .= '<div class="' . wp_rem_cs_var_page_builder_element_sizes($page_element_size, $atts) . ' ">';
        }
        ...
        $html .= wp_rem_plugin_title_sub_align($wp_rem_cs_locations_element_title, $wp_rem_cs_locations_element_subtitle, $wp_rem_var_locations_align, $wp_rem_location_element_title_color, $wp_rem_location_seperator_style, $wp_rem_location_element_subtitle_color);
        $page_url = isset($wp_rem_search_result_page) ? wp_rem_wpml_lang_page_permalink($wp_rem_search_result_page, 'page') : '';
        $view_class = '';
        ...
        if ( isset($all_locations) && ! empty($all_locations) && is_array($all_locations) ) {
            foreach ( $all_locations as $value ) {
                $location = get_term_by('slug', $value, 'wp_rem_locations');
        ...
        if ( ! empty($wp_rem_all_locations_url) && $wp_rem_var_locations_style == 'modern' ) {
            $html .='<a href="' . esc_url($wp_rem_all_locations_url) . '" class="view-loc-btn">' . wp_rem_plugin_text_srt('wp_rem_locations_view_all_locations') . '</a>';
        }
        $html .= '</div>';
        if ( function_exists('wp_rem_cs_var_page_builder_element_sizes') ) {
            $html .= '</div>';
