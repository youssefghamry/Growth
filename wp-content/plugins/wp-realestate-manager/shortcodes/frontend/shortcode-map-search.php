...
if (!class_exists('Wp_rem_Shortcode_Map_Search_front')) {
	...
	public function wp_rem_map_search_shortcode_callback($atts, $content = "") {
	    ...
	    $wp_rem_drawing_tools = isset($wp_rem_plugin_options['wp_rem_drawing_tools']) ? $wp_rem_plugin_options['wp_rem_drawing_tools'] : '';
	    ...
	    $wp_rem_search_result_page = ( $wp_rem_search_result_page != '' ) ? wp_rem_wpml_lang_page_permalink($wp_rem_search_result_page, 'page') : '';
			...
			?>
				...
				<div class="top-map-search-inner<?php echo esc_html($search_box_class) ?>"> 
					...
					<div class="main-search modern">
						...
						$wp_rem_post_property_typez = new Wp_rem_Post_Property_Types();
						$property_types_ar = $wp_rem_post_property_typez->wp_rem_types_array_callback('NULL');
						$type_count = count($property_types_ar);


						if ($propertysearch_property_type_switch == 'yes' && $type_count <= 2) {
						    ?>
						    ...
						    $number_option_flag = 1;
						    ?>
		    				<div id="property_type_select_fields_<?php echo wp_rem_allow_special_char($rand_numb); ?>" class="property-type-cate-fields field-holder select-dropdown has-icon">
		    				    <label>
							    <?php
							    $wp_rem_post_property_types = new Wp_rem_Post_Property_Types();
							    $property_types_array = $wp_rem_post_property_types->wp_rem_types_array_callback('NULL');
								...
								'options' => $types_array,
								'extra_atr' => ' onchange="wp_rem_property_type_search_fields(this,\'' . $rand_numb . '\',\'' . $propertysearch_price_switch . '\'); wp_rem_property_type_cate_fields(this,\'' . $rand_numb . '\',\'' . $propertysearch_categories_switch . '\'); "',
							    );
						...
						if ($propertysearch_location_switch == 'yes') {
						    ...
						    do_action('homevillas_search_location_filters', '<div class="field-holder search-input"><div id="wp-rem-top-select-holder" class="search-country" style="display:' . wp_rem_allow_special_char($wp_rem_select_display) . '"><div class="select-holder">', '</div></div></div>', false, $rand_numb, 'filter', 'maps', 'wp_rem_top_serach_trigger();');
						}
	...
	public function map_search_query($atts = '') {
	    ...
	    $property_sort_order = 'desc';   // default value
            $qryvar_property_sort_type = 'desc';  // default value
            $qryvar_sort_by_column = 'post_date';  // default value
	    ...
	    $wp_rem_map_marker_icon = isset($wp_rem_plugin_options['wp_rem_map_marker_icon']) && $wp_rem_plugin_options['wp_rem_map_marker_icon'] != '' ? $wp_rem_plugin_options['wp_rem_map_marker_icon'] : wp_rem::plugin_url() . '/assets/frontend/images/map-marker.png';
	    $wp_rem_map_cluster_icon = isset($wp_rem_plugin_options['wp_rem_map_cluster_icon']) && $wp_rem_plugin_options['wp_rem_map_cluster_icon'] != '' ? $wp_rem_plugin_options['wp_rem_map_cluster_icon'] : wp_rem::plugin_url() . '/assets/frontend/images/map-cluster.png';
	    $drawing_tools_line_color = isset($wp_rem_plugin_options['wp_rem_drawing_tools_line_color']) && $wp_rem_plugin_options['wp_rem_drawing_tools_line_color'] != '' ? $wp_rem_plugin_options['wp_rem_drawing_tools_line_color'] : '#1e90ff';
	    $drawing_tools_fill_color = isset($wp_rem_plugin_options['wp_rem_drawing_tools_fill_color']) && $wp_rem_plugin_options['wp_rem_drawing_tools_fill_color'] != '' ? $wp_rem_plugin_options['wp_rem_drawing_tools_fill_color'] : '#1e90ff';
	    if ($wp_rem_map_marker_icon != '') {
		$wp_rem_map_marker_icon = wp_get_attachment_url($wp_rem_map_marker_icon);
	    } else {
		$wp_rem_map_marker_icon = esc_url(wp_rem::plugin_url() . 'assets/frontend/images/map-marker.png');
		...
		'full_screen' => wp_rem_plugin_text_srt('wp_rem_map_full_screen_text'),
		'exit_full_screen' => wp_rem_plugin_text_srt('wp_rem_map_exit_full_screen_text'),
	    );
	    if (isset($location_coordinates) && !empty($location_coordinates)) {
