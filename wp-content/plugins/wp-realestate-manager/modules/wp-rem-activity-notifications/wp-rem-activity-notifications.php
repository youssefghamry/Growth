...
if (!class_exists('Wp_rem_Activity_Notifications')) {
    ...
    class Wp_rem_Activity_Notifications {
        ...
        public function wp_rem_enqueue_files_frontend_callback() {
            wp_register_script('wp-rem-notifications-js', plugins_url('/assets/js/functions.js', __FILE__), '', '', true);
        }
         ...
         */
        public function wp_rem_addon_notification_text_strings_callback($wp_rem_static_text = array()) {

			if ( ! is_array($wp_rem_static_text) ) {
				$wp_rem_static_text = array();
			}
