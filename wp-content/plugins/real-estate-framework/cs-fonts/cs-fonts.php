...
if ( ! class_exists( 'wp_rem_google_fonts' ) ) {
	...
	class wp_rem_google_fonts {
		...
		function __construct() {
			...
			define( 'FRAMEWORK_CORE_DIR', WP_PLUGIN_DIR . '/real-estate-framework/cs-fonts' );
				...
				<?php if( isset($_REQUEST['export']) && $_REQUEST['export'] == '1' ){ ?>
					<div class="export-fonts-wrapper">
						<div class="export-btn">
							<a class="export-fonts-btn" href="javascript:void(0);"><?php echo wp_rem_cs_var_frame_text_srt( 'wp_rem_fonts_export_btn_label' ); ?></a>
							<a id="export-fonts" class="export-fonts" href="javascript:void(0);" style="display:none;" download=""><?php echo wp_rem_cs_var_frame_text_srt( 'wp_rem_fonts_export_btn_label' ); ?></a>
						</div>
					</div>
				<?php } ?>
				<div class="clear"></div>
			...
			<div class="wp-rem-fonts wp-rem-custom-fonts">
				...
				<div class="custom-fonts-list-holder">
					...
					if ( isset( $wp_rem_custom_fonts_list['name'] ) && is_array( $wp_rem_custom_fonts_list ) && ! empty( $wp_rem_custom_fonts_list['name'] ) ) {
						foreach ( $wp_rem_custom_fonts_list['name'] as $key => $wp_rem_custom_font ) {
							...
							if ( isset( $wp_rem_selected_custom_fonts['name'] ) && ! empty( $wp_rem_selected_custom_fonts['name'] ) && in_array( $font_name, $wp_rem_selected_custom_fonts['name'] ) ) {
				...
				<div class="added-custom-fonts-list">
					...
					<div id="custom-fonts-selected-wrapper">
						<?php 
						if ( isset( $wp_rem_selected_custom_fonts['name'] ) && is_array( $wp_rem_selected_custom_fonts ) && ! empty( $wp_rem_selected_custom_fonts['name'] ) ) {
							foreach ( $wp_rem_selected_custom_fonts['name'] as $key => $wp_rem_custom_font ) {
								...
								$font_woff = isset( $wp_rem_selected_custom_fonts['woff'][$key] ) ? $wp_rem_selected_custom_fonts['woff'][$key] : '';
								$font_ttf = isset( $wp_rem_selected_custom_fonts['ttf'][$key] ) ? $wp_rem_selected_custom_fonts['ttf'][$key] : '';
								$font_svg = isset( $wp_rem_selected_custom_fonts['svg'][$key] ) ? $wp_rem_selected_custom_fonts['svg'][$key] : '';
								$font_eot = isset( $wp_rem_selected_custom_fonts['eot'][$key] ) ? $wp_rem_selected_custom_fonts['eot'][$key] : '';
								if (( isset( $font_woff ) && $font_woff != '' ) && ( isset( $font_woff ) && $font_woff != '' ) && ( isset( $font_woff ) && $font_woff != '' ) && ( isset( $font_woff ) && $font_woff != '' ) ){
								$font_face_html = "<style type=\"text/css\">
									@font-face {
									font-family: '". $font_name ."';
