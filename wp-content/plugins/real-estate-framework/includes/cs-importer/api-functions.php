 ...
 */
if ( ! function_exists('wp_rem_add_to_active_themes_callback') ) {

    function wp_rem_add_to_active_themes_callback( $selected_demo = '', $demo_data_status = 'complete', $ajax_reques = 'no') {
    ...
    add_action('wp_ajax_wp_rem_add_to_active_themes', 'wp_rem_add_to_active_themes_callback');
}
...
if ( ! function_exists('wp_rem_release_purchase_code_callback') ) {

    function wp_rem_release_purchase_code_callback() {
        $remote_api_url = REMOTE_API_URL;
        $envato_purchase_code_verification = get_option('item_purchase_code_verification');
        $purchase_code = isset($envato_purchase_code_verification['item_puchase_code']) ? $envato_purchase_code_verification['item_puchase_code'] : '';
        $update_data = array(
    ...
    add_action('wp_ajax_wp_rem_release_purchase_code', 'wp_rem_release_purchase_code_callback');
}