...
if ( ! function_exists('wp_rem_pl_opt_backup_generate') ) {
    ...
    function wp_rem_pl_opt_backup_generate() {
        ...
        $wp_rem_export_options = apply_filters('wp_rem_translate_options', $wp_rem_export_options );
        ...
        $wp_rem_filename = trailingslashit($wp_rem_upload_dir) . (current_time('d-M-Y_H.i.s', 1)) . '.json';
...
if ( ! function_exists('wp_rem_demo_plugin_data') ) {
    ...
    function wp_rem_demo_plugin_data($filename = "", $obj = array()) {
        ...
        if ( $filename == "" ) {
            ...
            if ( isset($obj) && ! empty($obj) ) {
                ...
                $wp_rem_settings = str_replace(str_replace('"', '', json_encode($obj->attachments_replace_url)), str_replace('"', '', json_encode($obj->wp_upload_url_path)), $wp_rem_settings);
                //
            }
            $plugin_settings = json_decode($wp_rem_settings, true);
            $demo_plugin_data = $plugin_settings;
    ...
    function update_fixed_price_opt($post) {
        ...
        if ( $fixed_price_opt != '' ) {
            ...
            foreach ( $fixed_price_opt as $key => $label ) {
                $post['fixed_price_opt'][$label] = $label;
            }
        }
        return $post;
    }
