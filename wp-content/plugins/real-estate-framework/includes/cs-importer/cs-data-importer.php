...
class wp_rem_cs_Data_Importer {
	...
	function __construct() {
		...
		$this->attachments_replace_url = ATTACHMENTS_REPLACE_URL;
	...
	function import() {
		...
		if ( $this->is_fonts ) {
			...
			ob_end_clean();
			if ( $this->action_return ) {
				$this->make_output( true, __( 'Fonts successfully got imported.', 'wp-rem-frame' ) );
			} else {
				$this->make_output( false, __( 'Sorry fonts file not readable.', 'wp-rem-frame' ) );
			}
		}
		...
		if ( $this->is_icons ) {
			...
			ob_end_clean();
			if ( $this->action_return ) {
				$this->make_output( true, __( 'Icons successfully got imported.', 'wp-rem-frame' ) );
			} else {
				$this->make_output( false, __( 'Sorry icons file not readable.', 'wp-rem-frame' ) );
			}
		}
	...
	function import_wp_data() {
		...
		if ( $this->demo_data_name == DEFAULT_DEMO_DATA_NAME ) {
			...
			if( $this->demo_data_name == 'rtl' ){
				$this->demo_data_name = $this->demo_data_name. '-demo';
			}else{
				$demo_name = explode( 'v', $this->demo_data_name );
				if( !empty($demo_name) && is_array($demo_name)){
					if( isset($demo_name[0]) && !empty($demo_name[0]) && isset($demo_name[1]) && !empty($demo_name[1])){
						if( $demo_name[1] > 5 ){
							$this->demo_data_name = 'demo-v'.$demo_name[1];
						}else{
							$this->demo_data_name = $demo_name[0]. '-v'. $demo_name[1];
						}
					}
				}
			}
...
//			$zip->close();
	...
	/**
	 * Import Fonts
	 * invoke wp_rem_import_fonts
	 */
	function import_fonts() {
		do_action( 'wp_rem_import_fonts', $this );
	}
	 ...
	 * invoke wp_rem_delete_default_content
	 */
	function delete_default_content() {
		do_action( 'wp_rem_delete_default_content' );
	}
	
	/**
	 * Update Db Fields
	 * invoke wp_rem_importer_db_update
	 */
	function update_db() {
		do_action( 'wp_rem_importer_db_update', $this );
	}
