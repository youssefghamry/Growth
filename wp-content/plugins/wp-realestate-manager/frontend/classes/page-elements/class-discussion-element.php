	...
	class wp_rem_discussion_element {
		...
		public function wp_rem_discussion_submit_callback() {
			...
			if ( $current_login_member == $property_member || $current_login_member == $enquiry_member ) {
				...
				$member_name ='<a href="'. get_the_permalink($sender_id). '">'. $member_name .'</a>';
				 ...
				 * Adding Notification
				 */
				$notification_type = 'enquiry_message';
				$notification_array = array(
					'type' => $notification_type,
					'element_id' => $comment_post_ID,
					'sender_id' => $sender_id,
					'reciever_id' => $reciever_id,
					'message' => $member_name . ' ' . wp_rem_plugin_text_srt( 'wp_rem_discussion_sent_you_message' ) . ' <a href="javascript:void();" onclick="javascript:wp_rem_enquiry_detail(\'' . $comment_post_ID . '\',\'my\');">' . wp_trim_words( get_the_title( $comment_post_ID ), 3 ) . '</a>',
				);
					...
					<div class="text-holder">
						<div class="heading">
							...
							<span datetime="<?php echo date( 'Y/m/d', strtotime( $comment->comment_date ) ); ?>" class="post-date">
								<?php echo human_time_diff( strtotime( $comment->comment_date ), current_time( 'timestamp', 1 ) ) . ' ' . wp_rem_plugin_text_srt( 'wp_rem_discussion_message_ago' ); ?>
							</span>
						</div>
