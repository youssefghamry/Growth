...
if ( ! class_exists( 'price_tables_post_type_meta' ) ) {
    ...
    class price_tables_post_type_meta {
            ...
            <div class="wp-rem-price-table">
                ...
                <table id="wp-rem-price-table-<?php echo absint( $rand_numb ) ?>" data-id="<?php echo absint( $rand_numb ) ?>" cellpadding="5" cellspacing="0" border="1">
                    <tbody>
                        ...
                        /* Fetching the price table ROWS SECTION START */
                        if(is_array($pt_row_title) && is_array($pt_col_input)){
                            $total_title = count($pt_row_title);
                            $total_col_input = count($pt_col_input);
                            $input_ina_row  = 0;
                            if( $total_title > 0){
                                $input_ina_row = $total_col_input / $total_title;
                            }
                            $start = 0;
                            $table_row = '';
                            $pt_index_counter = 0;
                            foreach ($pt_row_title as $titke_key => $title){
                                $table_row .= '<tr class="pt_row"><td class="pt_row_actions"><a class="pt_delete_row">x</a><br><input type="text" name="dir_pt_row_title[]" value="' . $title . '"></td>';
                                $end = (($titke_key + 1)* $input_ina_row) - 1;
                                $hidden_field_row_num = '<input type="hidden" class="row_num_input" name="dir_pt_row_num[]">';
                                for ($i = $start; $i <= $end; $i++){
                                    $rand_numb = rand( 10000000, 99999999 );
                                    $pt_sub_input_val = isset( $pt_col_sub_input[$pt_index_counter] ) ? $pt_col_sub_input[$pt_index_counter] : '';
                                    $table_row .= '<td><input type="text" name="dir_pt_col_val[]" size="10" value="'.$pt_col_input[$i].'"> <br> ' . $this->wp_rem_pt_icon( $pt_sub_input_val, $rand_numb, 'dir_pt_col_sub' ) . $hidden_field_row_num. '</td>';
                                    /*once in a row hidden field row number*/
                                    $hidden_field_row_num = '';
                                    if($i == $end){
                                        $start = $end + 1;
                                    }
                                    $pt_index_counter++;
                        ...
                        /* Fetching the price table ROWS SECTION END */
        ...
        public function wp_rem_insert_price_table_metas( $post_id ) {
            ...
            $row_num_input = isset( $_POST['dir_pt_row_num'] )? $_POST['dir_pt_row_num'] : array();
            $pt_col_input = isset( $_POST['dir_pt_col_val'] )? $_POST['dir_pt_col_val'] : array();
            $pt_col_sub_input = isset( $_POST['dir_pt_col_sub'] )? $_POST['dir_pt_col_sub'] : array();
            $pt_row_title = isset( $_POST['dir_pt_row_title'] )? $_POST['dir_pt_row_title'] : array();
            $pt_sec_val = isset( $_POST['dir_pt_sec_val'] )? $_POST['dir_pt_sec_val'] : array();
            $pt_sec_pos = isset( $_POST['dir_pt_sec_pos'] )? $_POST['dir_pt_sec_pos'] : array();

            $pt_pkg_name = isset( $_POST['pt_pkg_name'] )? $_POST['pt_pkg_name'] : array();
            $pt_pkg_price = isset( $_POST['pt_pkg_price'] )? $_POST['pt_pkg_price'] : array();
            $pt_pkg_color = isset( $_POST['pt_pkg_color'] )? $_POST['pt_pkg_color'] : array();
            $pt_pkg_desc = isset( $_POST['pt_pkg_desc'] )? $_POST['pt_pkg_desc'] : array();
            $pt_pkg_btn_txt = isset( $_POST['pt_pkg_btn_txt'] )? $_POST['pt_pkg_btn_txt'] : array();
            $pt_pkg_dur = isset( $_POST['pt_pkg_dur'] )? $_POST['pt_pkg_dur'] : array();
            $pt_pkg_feat = isset( $_POST['pt_pkg_feat'] )? $_POST['pt_pkg_feat'] : array();
            $pt_pkg_url = isset( $_POST['pt_pkg_url'] )? $_POST['pt_pkg_url'] : array();
			...
			if(is_array($row_num_input)){
                // saving num of rows
                update_post_meta( $post_id, 'wp_rem_pt_row_num', absint( sizeof( $row_num_input ) ) );
            }else{
                // saving num of rows           
            update_post_meta( $post_id, 'wp_rem_pt_row_num', $row_num_input);
            }
