                ...
                if ($isRegistrationOn) {
                    ...
                    <div class="footer-element comment-form-social-connect social_login_ui <?php if (strpos($_SERVER['REQUEST_URI'], 'wp-signup.php')) echo 'mu_signup'; ?>">
                        <div class="social_login_facebook_auth">
                            ...
                            if (!get_transient('wp_rem_login_fb_session_state')) {
                                set_transient('wp_rem_login_fb_session_state', $fb_session_state, 60 * 60 * 24 * 30);
                            } else {
                                $fb_session_state = get_transient('wp_rem_login_fb_session_state');
                            }
                            $wp_rem_opt_array = array(
                                'id' => '',
                            ...
                            <ul>	 
                                ...
                                if (is_user_logged_in()) {
                                    ...
                                    if ($google_enabled == 'on') :
                                        ...
                                        if ($signup == 'yes') {
                                            echo apply_filters('social_login_login_google', '<li><a  href="javascript:void(0);" id="googlesignup' . $google_rand_id . '" rel="nofollow" title="' . esc_html__('google-sinin', 'wp-rem') . '" data-original-title="google-plus" class=" gplus"><span class="social-mess-top gplus-social-login" style="display:none">' . esc_html__('Please set API key', 'jobhunt') . '</span><i class="icon-google4"></i>Sign In With Google</a></li>');
                                        } else {
                                            echo apply_filters('social_login_login_google', '<li><a  href="javascript:void(0);" id="googlesignin' . $google_rand_id . '" rel="nofollow" title="' . esc_html__('google-sinin', 'wp-rem') . '" data-original-title="google-plus" class=" gplus"><span class="social-mess-top gplus-social-login" style="display:none">' . esc_html__('Please set API key', 'jobhunt') . '</span><i class="icon-google4"></i>Sign In With Google</a></li>');
                                        }
                                        $cs_google_client_id = isset($wp_rem_plugin_options['wp_rem_google_client_id']) ? $wp_rem_plugin_options['wp_rem_google_client_id'] : '';
                                        ?><?php if ($signup == 'yes') { ?>
                                            <script>
 ...
 */
if (!function_exists('wp_rem_get_new_pass')) {

    function wp_rem_get_new_pass() {
        global $wpdb, $wp_hasher;
        if (isset($_POST['action']) && 'wp_rem_get_new_pass' == $_POST['action']) {
            ...
            if (empty($user_login)) {
                $json['type'] = "error";
                $json['msg'] = wp_rem_plugin_text_srt('wp_rem_forgot_pass_username_email_empty');
                ...
                if (empty($user_data)) {
                    $json['type'] = "error";
                    $json['msg'] = wp_rem_plugin_text_srt('wp_rem_forgot_pass_email_error');
                    echo json_encode($json);
                    wp_die();
                }
            } else {
                ...
                if (empty($user_data)) {
                    $json['type'] = "error";
                    $json['msg'] = wp_rem_plugin_text_srt('wp_rem_forgot_pass_username_error');
                    echo json_encode($json);
                    wp_die();
                }
            }
            ...
            if (!$allow) {
                $json['type'] = "error";
                $json['msg'] = wp_rem_plugin_text_srt('wp_rem_forgot_pass_not_allow');
                echo json_encode($json);
                wp_die();
            } else if (is_wp_error($allow)) {
                $json['type'] = "error";
                $json['msg'] = wp_rem_plugin_text_srt('wp_rem_forgot_pass_wp_error');
                echo json_encode($json);
                wp_die();
            }
            ...
            if (isset($type) && $type == 'page' && isset($current_page_id) && is_numeric($current_page_id)) {
                $reset_link = add_query_arg(array('reset_pass' => 'true', 'key' => $key, 'login' => rawurlencode($user_login), 'popup' => 'false'), esc_url(wp_rem_wpml_lang_page_permalink($current_page_id, 'page')));
            } else {
            ...
            $title = sprintf(wp_rem_plugin_text_srt('wp_rem_forgot_pass_title'), $blogname);
            ...
            do_action('wp_rem_confirm_reset_password', $args);

            $json['type'] = "success";
            $json['msg'] = wp_rem_plugin_text_srt('wp_rem_forgot_pass_link_sent');
            echo json_encode($json);
            wp_die();
        }
    }

    add_action('wp_ajax_wp_rem_get_new_pass', 'wp_rem_get_new_pass');
    add_action('wp_ajax_nopriv_wp_rem_get_new_pass', 'wp_rem_get_new_pass');
}

/*
 * Start Function  how to user  recover his  password
 */
if (!function_exists('wp_rem_reset_pass')) {

    function wp_rem_reset_pass() {
        global $wpdb, $wp_rem_plugin_options;

        $json = array();
        // check if we're in reset form
        if (isset($_POST['action']) && 'wp_rem_reset_pass' == $_POST['action']) {
            ...
            if ($random_password != $confirm_new_pass) {
                $json['type'] = "error";
                $json['msg'] = wp_rem_plugin_text_srt('wp_rem_forgot_pass_confirm_not_match');
                echo json_encode($json);
                wp_die();
            }
    ...
    add_action('wp_ajax_wp_rem_reset_pass', 'wp_rem_reset_pass');
    add_action('wp_ajax_nopriv_wp_rem_reset_pass', 'wp_rem_reset_pass');
}
...
if (!function_exists('wp_rem_lost_pass')) {
    ...
    function wp_rem_lost_pass($atts, $content = "") {
        ...
        if (isset($_GET['reset_pass']) && $_GET['reset_pass'] == 'true') {
            if (isset($_GET['key']) && $_GET['key'] != '' && isset($_GET['login']) && $_GET['login'] != '') {
                ...
                if ($user && $user->get_error_code() === 'expired_key') {
                    $error_msg = wp_rem_plugin_text_srt('wp_rem_forgot_pass_link_expired');
                    $reset_pass = false;
                } else if ($user && $user->get_error_code() === 'invalid_key') {
                    $error_msg = wp_rem_plugin_text_srt('wp_rem_forgot_pass_link_invalid');
                    $reset_pass = false;
                }
            }
        }
        ...
        if ($wp_rem_type == 'popup') {
            ...
            <span class="wp-rem-reset-link-error" style="color:red;"><?php echo esc_html($error_msg); ?></span>
            <div id="cs-result-<?php echo absint($wp_rem_rand) ?>"></div>
            <div class="login-form-id-<?php echo absint($wp_rem_rand) ?>">
                <form class="user_form" id="wp_pass_reset_<?php echo absint($wp_rem_rand) ?>" method="post" style="display:<?php echo esc_html($display_reset_pass_form); ?>;">		
                    <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
                        <div class="input-filed"> 
                            ...
                            $wp_rem_opt_array = array(
                                ...
                                'classes' => 'wp-rem-dev-req-field form-control',
                                'extra_atr' => 'placeholder="' . wp_rem_plugin_text_srt('wp_rem_forgot_pass_enter_new_pass') . '"',
                            );
                        ...
                        <div class="input-filed"> 
                            ...
                            $wp_rem_opt_array = array(
                                ...
                                'classes' => 'wp-rem-dev-req-field form-control',
                                'extra_atr' => 'placeholder="' . wp_rem_plugin_text_srt('wp_rem_forgot_pass_confirm_new_pass') . '"',
                            );
                            $wp_rem_form_fields_frontend->wp_rem_form_text_render($wp_rem_opt_array);
                ...
                <form class="user_form" id="wp_pass_lost_<?php echo absint($wp_rem_rand) ?>" method="post" style="display:<?php echo esc_html($display_lost_pass_form); ?>;">		
                    <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12">
                        <div class="input-filed"> 
                            ...
                            $wp_rem_opt_array = array(
                                ...
                                'cust_name' => "user_input",
                                'classes' => 'wp-rem-dev-req-field form-control',
                                'extra_atr' => ' onkeypress="wp_rem_contact_form_valid_press(this,\'email\')" placeholder="' . wp_rem_plugin_text_srt('wp_rem_forgot_pass_enter_username_email') . '"',
                            );
                            $wp_rem_form_fields_frontend->wp_rem_form_text_render($wp_rem_opt_array);
        ...
        <script type="text/javascript">
            var $ = jQuery;
            $("#wp_pass_reset_<?php echo absint($wp_rem_rand) ?>").submit(function () {

                var returnType = wp_rem_validation_process(jQuery("#wp_pass_reset_<?php echo absint($wp_rem_rand) ?>"));
                if (returnType == false) {
                    return false;
                }
                ...
                var input_data = $('#wp_pass_reset_<?php echo absint($wp_rem_rand) ?>').serialize() + '&action=wp_rem_reset_pass';
                $.ajax({
                    type: "POST",
                    ...
                    success: function (msg) {
                        ...
                        if (msg.type === 'success') {
                            jQuery('#wp_pass_reset_<?php echo absint($wp_rem_rand) ?> input[name=user_input]').val('');
                            jQuery('#wp_pass_reset_<?php echo absint($wp_rem_rand) ?> input[name=confirm_new_pass]').val('');
                        }
            ...
            $("#wp_pass_lost_<?php echo absint($wp_rem_rand) ?>").submit(function () {
                ...
                var returnType = wp_rem_validation_process(jQuery("#wp_pass_lost_<?php echo absint($wp_rem_rand) ?>"));
                if (returnType == false) {
                    return false;
                }
                var data_id = jQuery(this).closest('.login-form').data('id');
                ...
                if (typeof popup_class !== 'undefined' && popup_class === 'modal fade active in') {
                    var input_data = $('#wp_pass_lost_<?php echo absint($wp_rem_rand) ?>').serialize() + '&action=wp_rem_get_new_pass&type=popup';
                } else if (typeof popup_class === 'undefined') {
                    var input_data = $('#wp_pass_lost_<?php echo absint($wp_rem_rand) ?>').serialize() + '&action=wp_rem_get_new_pass&type=page';
                } else {
                    var input_data = $('#wp_pass_lost_<?php echo absint($wp_rem_rand) ?>').serialize() + '&action=wp_rem_get_new_pass&type=popup';
                }
                $.ajax({
                    ...
                    success: function (msg) {
                        ...
                        if (msg.type === 'success') {
                            ...
                            jQuery('#wp_pass_lost_<?php echo absint($wp_rem_rand) ?> input[name=user_input]').val('');
                        }
                        wp_rem_show_response(msg, '', thisObj);
                    }
                });
