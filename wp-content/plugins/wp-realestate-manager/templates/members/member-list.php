...
if ( $member_loop_obj->have_posts() ) {
    ...
    <div class="member-property member-medium">
        ...
        while ( $member_loop_obj->have_posts() ) : $member_loop_obj->the_post();
            global $post, $wp_rem_member_profile,$wp_rem_author_info;
            $post_id = $post;
            ...
            <div class="member-post" itemprop="performer" itemscope="" itemtype="<?php echo force_balance_tags($http_request); ?>schema.org/Person">
                ...
                <div class="img-holder">
                    <figure>
                        <a title="<?php echo esc_html(get_the_title($member_id)); ?>" href="<?php the_permalink(); ?>">
                            <?php
                            if ( $member_image != '' ) {
                                $no_image = '<img itemprop="image" src="' . $member_image . '" alt=""/>';
                ...
                <div class="text-holder">
                    ...
                    <?php if ( isset($wp_rem_post_loc_address_member) && $wp_rem_post_loc_address_member != '' ) { ?>
                        <span itemprop="address" class="member-address"><i class="icon-location-pin2"></i><?php echo esc_html($wp_rem_post_loc_address_member); ?> </span>
                    <?php } ?>   
                    <?php
                    ...
                    <?php if ( $wp_rem_biography != '' ) { ?>
                        <p itemprop="disambiguatingDescription"><?php echo esc_html(wp_trim_words($wp_rem_biography, $member_excerpt_length)); ?></p>
                        <?php
                    }
                    ?>
                    <ul class="member-info">
                        ...
                        if ( $wp_rem_phone_number != '' ) {
                            $all_data = '';
                            $all_data = $wp_rem_author_info->wp_rem_member_member_phone_num($wp_rem_phone_number,'icon-phone2' ,'<li>', '</li>', 'itemprop="telephone"');
