...
if ( ! class_exists('Wp_rem_Shortcode_Pricing_Table_front') ) {
	...
	class Wp_rem_Shortcode_Pricing_Table_front {
		...
		function combine_pt_section($keys, $values) {
			...
			array_walk($result, function(&$v) use ($result) {return $v = (count($v) == 1)? array_pop($v): $v;});
			return $result;
		}
		...
		public function wp_rem_pricing_table_shortcode_callback($atts, $content = "") {
			...
			$wp_rem_pricing_table_element_title_color = isset($atts['wp_rem_pricing_table_element_title_color']) ? $atts['wp_rem_pricing_table_element_title_color'] : '';
			$wp_rem_pricing_table_element_subtitle_color = isset($atts['wp_rem_pricing_table_element_subtitle_color']) ? $atts['wp_rem_pricing_table_element_subtitle_color'] : '';
			$wp_rem_pricing_plan_seperator_style = isset($atts['wp_rem_pricing_plan_seperator_style']) ? $atts['wp_rem_pricing_plan_seperator_style'] : '';
			...
			if ( isset($_POST['wp_rem_package_buy']) && $_POST['wp_rem_package_buy'] == '1' ) {
				...
				if ( is_user_logged_in() && (current_user_can('wp_rem_member') || current_user_can('administrator')) ) {
			...
			if ( $pricing_table_title != '' || $pricing_tabl_subtitle != '' ) {
				...
				$wp_rem_element_structure = wp_rem_plugin_title_sub_align($pricing_table_title, $pricing_tabl_subtitle, $pricing_table_title_align, $wp_rem_pricing_table_element_title_color, $wp_rem_pricing_plan_seperator_style, $wp_rem_pricing_table_element_subtitle_color);
			...
			if ( $pricing_table_id != '' ) {
				...
				$counter = 0;
				$internal_counter = 1;
				$number_of_pack = is_array($pt_pkg_name) && count($pt_pkg_name);
				$number_of_rows = is_array($pt_row_title) && count($pt_row_title);
				$totla_features = is_array($pt_col_input) && count($pt_col_input);
				
				$number_of_cols = '';
				$row_numbers = 0;
				if($totla_features != '' && $number_of_pack != ''){
					$number_of_cols = $totla_features / $number_of_pack;
				}
				
				if($totla_features != '' && $number_of_rows != ''){
					$row_numbers = $totla_features / $number_of_rows;
				}
				
				$new_col = 0;
				if ( is_array($pt_col_input) && $row_numbers > 0 ) {
					$chunked_array = (array_chunk($pt_col_input, $row_numbers));
				}
				if ( is_array($pt_col_sub_input) && $row_numbers > 0 ) {
										...
										<thead>
											...
											if ( ! empty($pt_pkg_name) ) {
												...
												foreach ( $pt_pkg_name as $key => $val ) {
													...
													if ( $featured == 'yes' ) {
														$output .= '<td class="active"><span class="popular-plan text-color">' . wp_rem_plugin_text_srt('wp_rem_price_plan_most_popular') . '</span></td>' . "\n";
														$featured_flag = true;
													} else {
														$output .= '<td></td>' . "\n";
													}
